...
import { 
// BsVolumeOffFill,
BsVolumeMuteFill, BsVolumeDownFill, BsVolumeUpFill, BsFullscreen, BsFullscreenExit, } from "react-icons/bs";
...
function VideoComponenteReact(props) {
    ...
    function onLoadedData() {
        ...
        if (refVideo && refVideo.current) {
            ...
            if (percentSeen > 0 && percentSeen < 100) {
                ...
                let continuar = confirm(`Anteriormente llevabas un ${percentSeen}% visto del video, deseas continuar viendolo en ${strTime}  `);
                if (continuar) {
    ...
    function onTimeUpdatatedVideo() {
        if (refVideo && refVideo.current) {
            if (!refVideo.current.currentTime)
                return;
            if (!refVideo.current.duration)
                return;
            const totalDuration = refVideo.current.duration;
    ...
    const IconVolumenMemo = useMemo(() => {
        ...
        if (volumePercentage === 0)
            return <BsVolumeMuteFill size={btnIconSize}/>;
        if (volumePercentage < 50)
            return <BsVolumeDownFill size={btnIconSize}/>;
    ...
    onMouseMove={onMouseOverContainerVideo} ref={refContainerVideo}>
    ...
    //   setIsPlayed(false)
    // }}
    onClick={onPlayPauseVideo} onDoubleClick={onFullScreen} onLoadedData={onLoadedData} onTimeUpdate={onTimeUpdatatedVideo} onProgress={onProgress} onCanPlayThrough={onCanPlayThrough} onLoad={onLoadVideo} 
    // onKeyDown={onKeyDownContainerVideo}
    // onExitFullScreen
    // poster={"https://upload.wikimedia.org/wikipedia/commons/b/b6/Image_created_with_a_mobile_phone.png"}
    id="videoUpload" src={videoUrl}></video>
...
const formatTimeStr = (h, m, s) => `${formatTime(h)}:${formatTime(m)}:${formatTime(s)}`;
